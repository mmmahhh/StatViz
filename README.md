...
### 3. 常见问题与排查
- **导出失败（空白或截断）**：html2canvas 在跨域/Canvas 受限环境中可能失败；可尝试使用服务端渲染导出，或配置代理/ CORS。生产环境建议在 Node 环境调用 jsPDF 生成 PDF 以规避浏览器安全限制。
- **图表不渲染/数据不更新**：检查 Y 轴字段是否为空、数值列是否全为 null；确保数据集已正确加载。
- **分组/筛选异常**：当 Y 轴变更时，若需重置分组字段请刷新页面（当前为简化实现，Y 轴变更未自动清理分组字段）。
